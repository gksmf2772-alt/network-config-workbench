...
test("app shell exposes separated summary review compare profile and report tabs", () => {
  ...
  assert.match(shell, /graphPageTabBtn/);
  assert.match(shell, /reportPageTabBtn/);
  assert.match(shell, /id="objectsTab"/);
  assert.match(shell, /id="graphTab"/);
  assert.match(shell, /id="reportTab"/);
  assert.match(selectors, /graphPageTabBtn: doc\.querySelector\("#graphPageTabBtn"\)/);
  assert.match(selectors, /graphReport: doc\.querySelector\("#graphReport"\)/);
  assert.match(legacy, /selectors\.objectsPageTabBtn\?\.addEventListener\("click", \(\) => setActiveTab\("objects"\)\)/);
  assert.match(legacy, /selectors\.graphPageTabBtn\?\.addEventListener\("click", \(\) => setActiveTab\("graph"\)\)/);
...
test("report tab has quick actions for summary review graph and export", () => {
  ...
  const selectors = fs.readFileSync("src/core/legacySelectors.js", "utf8");
  const legacy = fs.readFileSync("src/core/legacyCore.js", "utf8");
  const graph = fs.readFileSync("src/components/graph/RelationshipGraph.jsx", "utf8");
  const graphBridge = fs.readFileSync("src/components/graph/RelationshipGraphBridge.jsx", "utf8");
  const graphAdapter = fs.readFileSync("src/utils/graphAdapter.js", "utf8");
  const graphLayout = fs.readFileSync("src/utils/dagreLayout.js", "utf8");
  ...
  assert.match(legacy, /renderStandaloneGraphPage/);
  assert.match(legacy, /data-graph-mode-toggle="comparison"/);
  assert.match(legacy, /data-graph-mode-toggle="internal"/);
  assert.match(legacy, /data-graph-detail-toggle/);
  assert.match(legacy, /data-graph-layout-mode="free"/);
  assert.match(legacy, /data-graph-layout-mode="radial"/);
  assert.match(legacy, /GRAPH_DEFAULT_VISIBLE_TYPES/);
  assert.match(legacy, /GRAPH_TYPE_FILTER_LABELS[\s\S]*\["port", "Port"\]/);
  assert.match(legacy, /GRAPH_TYPE_FILTER_LABELS[\s\S]*\["lag", "LAG"\]/);
  assert.match(legacy, /GRAPH_TYPE_FILTER_LABELS[\s\S]*\["interface", "Interface"\]/);
  assert.match(legacy, /GRAPH_TYPE_FILTER_LABELS[\s\S]*\["static", "Static"\]/);
  assert.match(legacy, /GRAPH_TYPE_FILTER_LABELS[\s\S]*\["pim", "PIM"\]/);
  assert.match(legacy, /data-graph-type-toggle="\$\{escapeHtml\(type\)\}"/);
  assert.match(legacy, /prepareRelationshipGraphDisplay/);
  assert.match(legacy, /toReactFlowData/);
  assert.match(legacy, /validateReactFlowData/);
  assert.match(legacy, /applyDagreLayout/);
  assert.match(legacy, /applyScatterLayout/);
  assert.match(legacy, /data-graph-flow-root/);
  assert.match(legacy, /mountRelationshipGraphReactRoots/);
  assert.match(legacy, /renderRelationshipGraphReactPanel/);
  assert.match(graph, /ReactFlow/);
  assert.match(graph, /MiniMap/);
  assert.match(graph, /Controls/);
  assert.match(graph, /onNodeClick/);
  assert.match(graph, /onPaneClick/);
  assert.match(graph, /buildFocusNeighborhood/);
  assert.match(graph, /PRIMARY_TOPOLOGY_EDGE_TYPES/);
  assert.match(graph, /DEFAULT_VISIBLE_NODE_TYPES/);
  assert.match(graph, /serviceAggregate/);
  assert.match(graph, /onServicesOpen/);
  assert.match(graph, /nodeTypeVisible/);
  assert.match(graph, /filteredOut/);
  assert.match(graph, /resetEdges/);
  assert.match(graphBridge, /createRoot/);
  assert.match(graphBridge, /window\.renderRelationshipGraphReactPanel/);
  assert.match(legacy, /onServicesOpen:\s*\(\)\s*=>\s*rerenderGraphView\(root,\s*\{\s*viewMode:\s*"detail"\s*\}\)/);
  assert.match(graphAdapter, /validateReactFlowData/);
  assert.match(graphLayout, /dagre\.layout/);
  assert.match(graphLayout, /applyRadialLayout/);
  assert.match(main, /registerLegacyRelationshipGraphRenderer/);
  assert.match(packageJson, /"@xyflow\/react"/);
  assert.match(packageJson, /"dagre"/);
  assert.match(legacy, /data-graph-expand/);
  ...
  assert.match(css, /\.report-graph-layout-toggle/);
  assert.match(css, /\.report-graph-type-filters/);
  assert.match(css, /\.report-graph-type-toggle:has\(input:not\(:checked\)\)/);
  assert.match(css, /\.report-graph-flow-root/);
  assert.match(css, /\.react-flow__minimap/);
  assert.match(css, /\.report-graph-standalone/);
  assert.match(css, /\.report-graph\.graph-expanded/);
