# Blocks Auto-Generation Analysis

Generated: 2026-05-18T12:21:00.163Z
Case: nokia-classic15-to-nokia-mdcli-22

## Summary
- baseline total: 71
- total: 0
- resolved from baseline: 71
- parser extension: 0
- conversion policy: 0
- target fixture completion: 0
- actual config correction: 0
- conclusion: Remaining blocks are undefined target policy references. Classic indirect static-route next-hop parser gaps are resolved in this pass.

## By Rule
- none

## By Resolution
- none

## Resolved From Baseline
- static-route.next-hop-invalid: 71, Classic indirect/tunnel-next-hop static routes now preserve next-hop for audit/migration review.

## Top Items
